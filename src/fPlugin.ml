...
let reldecl_of_tuple = function
  | (na, None, ty) -> RelDecl.LocalAssum (na, ty)
  | (na, Some e, ty) -> RelDecl.LocalDef (na, e, ty)
...
let force_translate_inductive cat ind ids =
  ...
  let substind =
    Array.map_to_list (fun oib -> NamedDecl.LocalAssum
                          (oib.mind_typename,
			   Preinductive.type_of_inductive env ((mib, oib), Sorts.Instance.empty)))
      mib.mind_packets
  in
...
(** À chaque inductif I on associe une fonction λp.λparams.λindices.λ(qα), 
  ...
  let (sigma, substfn) = 
    let fn_oib oib = 
      ...
      let fold (sigma, ctxt) decl =
        ...
        (sigma, reldecl_of_tuple (x, o, tr_t_) :: ctxt)
      in
      let (sigma, tr_arity) = List.fold_left fold (sigma, []) oib.mind_arity_ctxt in
      (sigma, it_mkLambda_or_LetIn fnbody
         ([RelDecl.LocalAssum (Anonymous, FTranslate.hom cat (mkRel 3) (mkRel (3 + narityctxt)));
           RelDecl.LocalAssum (Anonymous, obj)] @ tr_arity @
          [RelDecl.LocalAssum (Anonymous, obj)]))
    in
  ...
  let make_one_entry params body (sigma, bodies_) =
    ...
    let (sigma, s) =
      let evdref = ref sigma in
      let s =
        if Inductive.elim_sorts_abstract (mib,body) = Sorts.InType
        then Evarutil.e_new_Type env evdref
        else mkProp
      in
      !evdref, s
    in
    ...
    let fold_lc (sigma, lc_) typ =
      ...
      let envtyp_ = Environ.push_rel_context
          [RelDecl.LocalAssum (Name (Nameops.add_suffix body.mind_typename "_f"),
...
(*   List.iter debug bodies_; *)
  ...
  | RelDecl.LocalDef (na, body, _) -> (Nameops.out_name na, LocalDefEntry body)
  in
